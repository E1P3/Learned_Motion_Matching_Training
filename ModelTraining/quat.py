...
def forward_kinematics(lpos, lrot, parents, bone_index):
    if parents[bone_index] == -1:
        return lpos[bone_index], lrot[bone_index]
    else:
        ppos, prot = forward_kinematics(lpos, lrot, parents, parents[bone_index])
        return mul_vec(prot, lpos[bone_index]) + ppos, mul(prot, lrot[bone_index])
    
def forward_kinematics_velocities(lpos, lrot, lvel, lang, parents, bone_index):
    if parents[bone_index] == -1:
        return lpos[bone_index], lrot[bone_index], lvel[bone_index], lang[bone_index]
    else:
        ppos, prot, pvel, pang = forward_kinematics_velocities(lpos, lrot, lvel, lang, parents, parents[bone_index])
        return (
            mul_vec(prot, lpos[bone_index]) + ppos, 
            mul(prot, lrot[bone_index]),
            mul_vec(prot, lvel[bone_index]) + _fast_cross(pang, mul_vec(prot, lpos[bone_index])) + pvel,
            mul_vec(prot, lang[bone_index]) + pang)